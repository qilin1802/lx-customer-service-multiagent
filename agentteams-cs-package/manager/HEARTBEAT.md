...
1. Check `admin_dm_room_id` in state.json. If `null`, discover it now:
   - List joined rooms, find the DM room with exactly 2 members: you and `@admin:matrix-local.agentteams.io:18080`
   - Persist it:
     ```bash
...
- **Use `copaw channels send` via shell** to send a follow-up to that room:
  ...
  copaw channels send \
    --agent-id default \
    --channel matrix \
    --target-user "@{worker}:matrix-local.agentteams.io:18080" \
    --target-session "{room_id}" \
    --text "@{worker}:matrix-local.agentteams.io:18080 How is your current task {task-id} going? Are you blocked on anything?"
  ```
...
- **Use `copaw channels send` via shell** to send a follow-up to the Leader room:
  ...
  copaw channels send \
    --agent-id default \
    --channel matrix \
    --target-user "@{leader}:matrix-local.agentteams.io:18080" \
    --target-session "{room_id}" \
    --text "@{leader}:matrix-local.agentteams.io:18080 How is task {task-id} progressing? Any blockers from your team?"
  ```
...
2. Read `room_id` from the entry and **use `copaw channels send` via shell** to trigger execution:
   ...
   copaw channels send \
     ...
     --target-user "@{worker}:matrix-local.agentteams.io:18080" \
     --target-session "{room_id}" \
     --text "@{worker}:matrix-local.agentteams.io:18080 It's time to run your scheduled task {task-id} \"{task-title}\". Please execute it now and report back with the keyword \"executed\"."
   ```
...
- If the responsible Worker has had no activity during this heartbeat cycle, **ensure the Worker's container is running first** (`lifecycle-worker.sh --action ensure-ready --worker {worker}`), then **use `copaw channels send` via shell** to send a follow-up to the project room:
  ...
  copaw channels send \
    ...
    --text "@{worker}:matrix-local.agentteams.io:18080 Any progress on your current task {task-id} \"{title}\"? Please let us know if you're blocked."
  ```
...
- Resolve the notification channel:
  ...
  - When `channel` is **`matrix`**: set `--target-session` to the room id from `target` after stripping a leading `room:` prefix if present. Set `--target-user` to the admin's full Matrix id `@admin:matrix-local.agentteams.io:18080`. Run:
    ```bash
    copaw channels send \
      --agent-id default \
      --channel matrix \
      --target-user "@admin:matrix-local.agentteams.io:18080" \
      --target-session "<room_id_without_room_prefix>" \
      --text "@admin:matrix-local.agentteams.io:18080 [Heartbeat Report] <summarize findings and recommended actions, in SOUL.md persona and language>"
    ```
