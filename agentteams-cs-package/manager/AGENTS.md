...
- **Your workspace:** `~/` (SOUL.md, config.json, memory/, skills/, state.json — local only, host-mountable, never synced to MinIO)
...
- **MinIO prefix:** always use `agentteams/agentteams-storage` in mc commands (this env var is pre-set in your shell, format: `<mc-alias>/<bucket>`)
- **Example:** `mc mirror agentteams/agentteams-storage/shared/tasks/{task-id}/ /root/agentteams-fs/shared/tasks/{task-id}/ --overwrite`
- **NEVER guess or hardcode the prefix** — do NOT use `agentteams-fs/...`, `agentteams-storage/...`, or any literal path. Always use `agentteams/agentteams-storage`. If unsure, run `echo agentteams/agentteams-storage` to check.
...
- When assigning a task to a Worker: `@alice:matrix-local.agentteams.io:18080`
- When notifying the human admin in a project room: `@admin:matrix-local.agentteams.io:18080`
